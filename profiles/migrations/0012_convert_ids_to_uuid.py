...
def drop_fk_constraints(apps, schema_editor):
    ...
    already_handled = {
        ("profiles_speakerexperiences", "speaker_id"),
        ("profiles_speakerskilltag", "speaker_id"),
        ("profiles_speakersociallinks", "speaker_id"),
        ("profiles_speakerfollow", "speaker_id"),
        ("profiles_speakerprofile_events_spoken", "speakerprofile_id"),
    }
    for table_name, column_name, constraint_name in fk_constraints:
...
def convert_columns_to_uuid(apps, schema_editor):
    ...
    if schema_editor.connection.vendor == "postgresql":
        schema_editor.execute(
            "ALTER TABLE profiles_speakerprofile ALTER COLUMN id DROP IDENTITY IF EXISTS;"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerexperiences ALTER COLUMN id DROP IDENTITY IF EXISTS;"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerskilltag ALTER COLUMN id DROP IDENTITY IF EXISTS;"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakersociallinks ALTER COLUMN id DROP IDENTITY IF EXISTS;"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerfollow ALTER COLUMN id DROP IDENTITY IF EXISTS;"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerprofile ALTER COLUMN id TYPE uuid USING (gen_random_uuid());"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerexperiences ALTER COLUMN id TYPE uuid USING (gen_random_uuid());"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerskilltag ALTER COLUMN id TYPE uuid USING (gen_random_uuid());"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakersociallinks ALTER COLUMN id TYPE uuid USING (gen_random_uuid());"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerfollow ALTER COLUMN id TYPE uuid USING (gen_random_uuid());"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerexperiences ALTER COLUMN speaker_id DROP NOT NULL;"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerexperiences ALTER COLUMN speaker_id TYPE uuid USING (NULL);"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerskilltag ALTER COLUMN speaker_id DROP NOT NULL;"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerskilltag ALTER COLUMN speaker_id TYPE uuid USING (NULL);"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakersociallinks ALTER COLUMN speaker_id DROP NOT NULL;"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakersociallinks ALTER COLUMN speaker_id TYPE uuid USING (NULL);"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerfollow ALTER COLUMN speaker_id DROP NOT NULL;"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerfollow ALTER COLUMN speaker_id TYPE uuid USING (NULL);"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerprofile_events_spoken ALTER COLUMN speakerprofile_id DROP NOT NULL;"
        )
        schema_editor.execute(
            "ALTER TABLE profiles_speakerprofile_events_spoken ALTER COLUMN speakerprofile_id TYPE uuid USING (NULL);"
        )
...
def restore_fk_constraints(apps, schema_editor):
    ...
    speakers are optional, social links, follows, and the m2m through rows
    require a speaker.
    """
    if schema_editor.connection.vendor != "postgresql":
        return
    ...
    nullable_fks = [
        ("profiles_speakerexperiences", "speaker_id", "fk_speakerexperiences_speaker_uuid"),
        ("profiles_speakerskilltag", "speaker_id", "fk_speakerskilltag_speaker_uuid"),
    ]
    required_fks = [
        (
            "profiles_speakersociallinks",
        ...
        ("profiles_speakerfollow", "speaker_id", "fk_speakerfollow_speaker_uuid"),
        (
    ...
    for table_name, column_name, _ in required_fks:
        cursor.execute(
            f'ALTER TABLE "{table_name}" ALTER COLUMN "{column_name}" SET NOT NULL;'
        )
    for table_name, column_name, constraint_name in nullable_fks + required_fks:
        cursor.execute(
            f"""
        ...
        cursor.execute(
            ...
            FOREIGN KEY ("{column_name}") REFERENCES profiles_speakerprofile (id)
            ON DELETE CASCADE;
            """,
        )
