...
class CFPSubmissionListCreateView(ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if _is_event_organizer(user, event):
            return CFPSubmission.objects.filter(event=event).prefetch_related("co_speakers")
        return CFPSubmission.objects.filter(event=event, submitter=user).prefetch_related("co_speakers")
...
class CFPStatusUpdateView(UpdateAPIView):
    ...
    def get_object(self):
        ...
        if not _is_event_organizer(self.request.user, obj.event):
            raise PermissionDenied("Only event organizers can update submission status.")
        return obj
